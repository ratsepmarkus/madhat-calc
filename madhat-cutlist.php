...
function madhat_register_settings() {
    ...
    // Uued seaded
    register_setting('madhat_options_group', 'madhat_recaptcha_site_key');
    register_setting('madhat_options_group', 'madhat_recaptcha_secret_key');
    register_setting('madhat_options_group', 'madhat_enable_win');
    register_setting('madhat_options_group', 'madhat_enable_int');
    register_setting('madhat_options_group', 'madhat_info_placeholder');
}
...
function madhat_settings_page_html() {
    ...
    $enable_win = get_option('madhat_enable_win', '1');
    $enable_int = get_option('madhat_enable_int', '1');
    $placeholder = get_option('madhat_info_placeholder', 'Täpsustavad soovid / kaaspaketi tüüp / asukoht / tellingu vajadus / ligipääs jne...');
    ?>
    <div class="wrap">
        ...
        <form method="post" action="options.php" id="madhatAdminForm">
            ...
            <div style="display:grid; grid-template-columns: 1fr; gap:20px; max-width: 800px;">
                ...
                <div style="background:#fff; padding:20px; border:1px solid #ccd0d4; border-radius:4px;">
                    <h3>Üldised Seaded & Vormi Kuvamine</h3>
                    <table class="form-table">
                        ...
                        <tr valign="top">
                            <th scope="row">Kuvatavad materjalid:</th>
                            <td>
                                <label><input type="checkbox" name="madhat_enable_win" value="1" <?php checked(1, $enable_win); ?> /> Aknakiled valikus</label><br>
                                <label><input type="checkbox" name="madhat_enable_int" value="1" <?php checked(1, $enable_int); ?> /> Sisustuskiled valikus</label>
                            </td>
                        </tr>
                        <tr valign="top">
                            <th scope="row">Lisainfo lahtri tekst (Placeholder):</th>
                            <td>
                                <input type="text" name="madhat_info_placeholder" value="<?php echo esc_attr($placeholder); ?>" class="large-text" />
                            </td>
                        </tr>
                    </table>
                </div>
                ...
                <div style="background:#fff; padding:20px; border:1px solid #ccd0d4; border-radius:4px;">
                    <h3>Turvalisus (Google reCAPTCHA v3)</h3>
                    <p class="description">Jäta tühjaks, kui ei soovi spämmikaitset kasutada.</p>
                    <table class="form-table">
                        <tr valign="top">
                            <th scope="row">Site Key:</th>
                            <td><input type="text" name="madhat_recaptcha_site_key" value="<?php echo esc_attr(get_option('madhat_recaptcha_site_key')); ?>" class="regular-text" /></td>
                        </tr>
                        <tr valign="top">
                            <th scope="row">Secret Key:</th>
                            <td><input type="password" name="madhat_recaptcha_secret_key" value="<?php echo esc_attr(get_option('madhat_recaptcha_secret_key')); ?>" class="regular-text" /></td>
                        </tr>
                    </table>
                </div>

                <div style="background:#fff; padding:20px; border:1px solid #ccd0d4; border-radius:4px;">
                    <h3>Aknakiled (+20mm)</h3>
                    <table class="wp-list-table widefat fixed striped" id="table-win">
                        <thead>
                            <tr><th>Nimetus</th><th style="width:100px;">Min €/m²</th><th style="width:100px;">Max €/m²</th><th style="width:60px;"></th></tr>
                        </thead>
                        <tbody></tbody>
                    </table>
                    <button type="button" class="button action" onclick="addAdminRow('table-win')" style="margin-top:10px;">+ Lisa rida</button>
                </div>

                <div style="background:#fff; padding:20px; border:1px solid #ccd0d4; border-radius:4px;">
                    <h3>Sisustuskiled (+80mm)</h3>
                    ...
                    <button type="button" class="button action" onclick="addAdminRow('table-int')" style="margin-top:10px;">+ Lisa rida</button>
                </div>
...
// ---------------------------------------------------------
// 2. AJAX HINNA ARVUTUS (PEIDETUD ÄRISALADUS)
// ---------------------------------------------------------
add_action('wp_ajax_madhat_calc_price', 'madhat_ajax_calc_price_handler');
add_action('wp_ajax_nopriv_madhat_calc_price', 'madhat_ajax_calc_price_handler');

function madhat_ajax_calc_price_handler() {
    ...
    if (is_array($items)) {
        foreach($items as $i) {
            ...
            $mat_name = isset($parts[1]) ? $parts[1] : '';
            
            $prices = ['min'=>0, 'max'=>0];
            $dataArr = ($mat_type === 'win') ? $win_data : $int_data;
            foreach($dataArr as $row) {
                if($row['name'] === $mat_name) {
                    $prices = ['min'=>floatval($row['min']), 'max'=>floatval($row['max'])];
                    break;
                }
            }
...
function madhat_render_form() {
    ...
    $enable_win = get_option('madhat_enable_win', '1');
    $enable_int = get_option('madhat_enable_int', '1');
    $placeholder = get_option('madhat_info_placeholder', 'Täpsustavad soovid...');
    $site_key = get_option('madhat_recaptcha_site_key');

    // Kustutame hinnad ära enne JS-ile saatmist (Turvalisus)
    $safe_win = array_map(function($item) { return ['name' => $item['name']]; }, json_decode($json_win, true) ?: []);
    $safe_int = array_map(function($item) { return ['name' => $item['name']]; }, json_decode($json_int, true) ?: []);

    ob_start();
    ?>
    <style>
        .madhat-wrapper { max-width: 800px; margin: 40px auto; font-family: sans-serif; padding: 30px; background: #ffffff; border-radius: 12px; box-shadow: 0 10px 25px -5px rgba(0,0,0,0.1); color: #374151; }
        ...
        .madhat-input, .madhat-select, .madhat-textarea { width: 100%; padding: 8px 10px; border: 1px solid #d1d5db; border-radius: 6px; font-size: 15px; background-color: #f9fafb; }
        ...
        .item-row { background: #f8fafc; border: 1px solid #e2e8f0; border-radius: 8px; padding: 10px 15px; padding-top: 25px; margin-bottom: 10px; position: relative; }
        .row-number { position: absolute; top: 5px; left: 10px; font-size: 0.7rem; font-weight: 700; color: #94a3b8; text-transform: uppercase; }
        .measurements-header { display: grid; grid-template-columns: 2fr 1.5fr 1fr 1fr 0.6fr 40px; gap: 10px; margin-bottom: 5px; }
        .measurements-header label { font-size: 0.8rem; color: #6b7280; font-weight: 600; }
        .item-grid { display: grid; grid-template-columns: 2fr 1.5fr 1fr 1fr 0.6fr 40px; gap: 10px; align-items: center; }
        ...
        .btn-remove { background-color: #fee2e2; color: #ef4444; height: 38px; width: 100%; display: flex; align-items: center; justify-content: center; border: 1px solid #fecaca; padding: 0; border-radius: 6px; }
        ...
        .submit-area { margin-top: 20px; display: flex; align-items: center; justify-content: flex-start; gap: 20px; }
        .confirm-box { display: flex; align-items: center; gap: 8px; font-size: 0.95rem; cursor: pointer; }
        ...
        @media (max-width: 1200px) {
            ...
            .madhat-input, .madhat-select { padding: 6px 4px !important; font-size: 12px !important; }
            .item-grid { grid-template-columns: 1fr 1fr 0.8fr 40px; grid-template-areas: "mat mat name name" "width height qty del"; gap: 6px; }
            .grid-mat { grid-area: mat; } .grid-n { grid-area: name; } .grid-w { grid-area: width; } .grid-h { grid-area: height; } .grid-q { grid-area: qty; } .grid-d { grid-area: del; align-items: flex-end; display: flex; flex-direction: column; }
            .madhat-wrapper { padding: 20px 15px; }
            .btn-remove { margin-top: auto; height: 35px; }
            .submit-area { flex-direction: column-reverse; align-items: center; gap: 20px; }
    ...
    <div class="madhat-wrapper">
        <?php if (isset($_GET['madhat_sent']) && $_GET['madhat_sent'] == '1'): ?>
            ...
            <script>localStorage.removeItem('madhat_form_data_v3');</script>
        <?php endif; ?>
        ...
        <form action="<?php echo esc_url(admin_url('admin-post.php')); ?>" method="post" enctype="multipart/form-data" id="madhatForm">
            ...
            <div class="madhat-grid-2">
                <div>
                    ...
                    <input type="text" class="madhat-input" name="project_title" id="project_title" required placeholder="nt. Korteri sisustus" oninvalid="this.setCustomValidity('Palun täida see väli')" oninput="this.setCustomValidity('')">
                </div>
                <div>
                    <label class="madhat-label">Nimi (Kontaktisik)</label>
                    <input type="text" class="madhat-input" name="contact_name" id="contact_name" required placeholder="Sinu nimi" oninvalid="this.setCustomValidity('Palun täida see väli')" oninput="this.setCustomValidity('')">
                </div>
            </div>
            <div class="madhat-grid-2">
                <div>
                    <label class="madhat-label">E-post</label>
                    <input type="email" class="madhat-input" name="client_email" id="client_email" required placeholder="näide@email.ee" oninvalid="this.setCustomValidity('Palun täida see väli')" oninput="this.setCustomValidity('')">
                </div>
                <div>
                    <label class="madhat-label">Telefon</label>
                    <input type="tel" class="madhat-input" name="client_phone" id="client_phone" required placeholder="+372..." oninvalid="this.setCustomValidity('Palun täida see väli')" oninput="this.setCustomValidity('')">
                </div>
            </div>
            ...
            <div class="madhat-mb" style="margin-top:25px;">
                ...
                <div class="measurements-header">
                    <label>Materjal</label><label>Pinna nimetus</label><label>Laius (cm)</label><label>Kõrgus (cm)</label><label>Kogus</label><label></label>
                </div>
            ...
            <div class="madhat-summary" id="price-box" style="display:none;">
                ...
                <span class="price-range" id="price-display">Arvutan...</span>
            </div>
            ...
            <div class="madhat-mb">
                ...
                <textarea name="client_info" id="client_info" class="madhat-textarea" rows="3" placeholder="<?php echo esc_attr($placeholder); ?>" oninput="saveState()"></textarea>
            </div>
            ...
            <div class="submit-area">
                <button type="button" class="madhat-btn btn-submit" onclick="handleFormSubmit(event)">SAADA PÄRING</button>
                <label class="confirm-box" id="confirm-label">
                    <input type="checkbox" name="confirm_measurements" id="confirm_check" required oninvalid="this.setCustomValidity('Palun märgi see kast')" oninput="this.setCustomValidity('')">
    ...
    const WIN_DATA = <?php echo json_encode($safe_win); ?>;
    const INT_DATA = <?php echo json_encode($safe_int); ?>;
    const ENABLE_WIN = <?php echo $enable_win ? 'true' : 'false'; ?>;
    const ENABLE_INT = <?php echo $enable_int ? 'true' : 'false'; ?>;

    function buildOptions(selected = '') {
        let html = '<option value="" disabled selected>-- Vali materjal --</option>';
        if (ENABLE_WIN && WIN_DATA.length > 0) {
            html += '<optgroup label="Aknakiled">';
            WIN_DATA.forEach(row => {
                const val = 'win|' + row.name;
                const isSel = (val === selected) ? 'selected' : '';
                html += `<option value="${val}" ${isSel}>${row.name}</option>`;
            });
            html += '</optgroup>';
        }
        if (ENABLE_INT && INT_DATA.length > 0) {
    ...
    function saveState() {
        ...
        localStorage.setItem('madhat_form_data_v3', JSON.stringify(formData));
        triggerCalcPrice();
    }

    function restoreState() {
        const saved = localStorage.getItem('madhat_form_data_v3');
    ...
    function triggerCalcPrice() {
        ...
        calcTimeout = setTimeout(calcPriceServer, 500); // Debounce
    }

    async function calcPriceServer() {
        const items = [];
        document.querySelectorAll('.item-row').forEach(row => {
            items.push({
                mat: row.querySelector('.input-mat').value,
        ...
        fd.append('action', 'madhat_calc_price');
        fd.append('items', JSON.stringify(items));

        try {
            const res = await fetch(ajaxurl, { method: 'POST', body: fd });
            const data = await res.json();
            if (data.success && data.data.max > 0) {
                display.textContent = `€${data.data.min} - ${data.data.max} + KM`;
        ...
        div.innerHTML = `
            ...
            <div class="item-grid">
                <div class="grid-mat">
                    ...
                    <select class="madhat-select input-mat" name="items[${rIdx}][mat]" required onchange="saveState()" oninvalid="this.setCustomValidity('Palun täida see väli')" oninput="this.setCustomValidity('')">
                        ${optionsHtml}
                    </select>
                </div>
                <div class="grid-n">
                ...
                <div class="grid-w">
                    ...
                    <input type="number" step="1" class="madhat-input input-w" name="items[${rIdx}][w]" value="${w}" placeholder="Laius" required onkeydown="return event.key !== ',' && event.key !== '.'" onchange="saveState()" oninvalid="this.setCustomValidity('Palun täida see väli')" oninput="this.setCustomValidity('')">
                </div>
                <div class="grid-h">
                    <span class="mobile-label">Kõrgus (cm)</span>
                    <input type="number" step="1" class="madhat-input input-h" name="items[${rIdx}][h]" value="${h}" placeholder="Kõrgus" required onkeydown="return event.key !== ',' && event.key !== '.'" onchange="saveState()" oninvalid="this.setCustomValidity('Palun täida see väli')" oninput="this.setCustomValidity('')">
                </div>
                <div class="grid-q">
                    <span class="mobile-label">Kogus</span>
                    <input type="number" class="madhat-input input-q" name="items[${rIdx}][q]" value="${q}" required onchange="saveState()" oninvalid="this.setCustomValidity('Palun täida see väli')" oninput="this.setCustomValidity('')">
                </div>
                <div class="grid-d">
                    <span class="mobile-label">&nbsp;</span>
                    <button type="button" class="madhat-btn btn-remove" onclick="this.closest('.item-row').remove(); updateRowUI(); saveState();">${trashSvg}</button>
                </div>
            </div>
        `;
    ...
    function handleFormSubmit(e) {
        e.preventDefault();
        const form = document.getElementById('madhatForm');
...
function madhat_handle_submit() {
    ...
    // reCAPTCHA kontroll
    $secret_key = get_option('madhat_recaptcha_secret_key');
    if (!empty($secret_key) && isset($_POST['g-recaptcha-response'])) {
        ...
        $res = json_decode($verify_body);
        if (!$res->success || $res->score < 0.5) {
            wp_die('Spämmi kaitse blokeeris päringu (reCAPTCHA). Palun proovi uuesti või võta otse ühendust.');
        }
    }
